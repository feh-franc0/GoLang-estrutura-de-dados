...
// 	// Big-O Notation:
// 	// Append: Amortized O(1), Remoção: O(n), Busca: O(n), Ordenação: O(n log n)
...
// 		fmt.Println(e.Value)
// 	}
// 	// Big-O: Inserção/remoção O(1) com ponteiro, Busca O(n)
...
// 	fmt.Println("Dequeued:", head)
// 	fmt.Println("Fila restante:", queue)
// 	// Big-O: Inserção O(1), Remoção O(1)
...
// 	delete(hash, "java")
// 	fmt.Println("HashMap após delete:", hash)
// 	// Big-O: Acesso/Inserção/Remoção média O(1)
...
// 	root.Insert(12)
// 	root.PrintInOrder()
// 	// Big-O: Busca, Inserção, Remoção: O(log n) média, O(n) pior caso

// 	// === Grafos (representação com map de listas) ===
// 	fmt.Println("\n--- Grafo ---")
// 	graph := make(map[string][]string)
// 	graph["A"] = []string{"B", "C"}
// 	graph["B"] = []string{"D"}
// 	graph["C"] = []string{"E"}
// 	fmt.Println("Vizinhos de A:", graph["A"])
// 	// Big-O: Depende da estrutura (adjacência: O(1) para checar conexão)
...
// 	fmt.Println("Contém 'go'?", trie.Search("go"))
// 	fmt.Println("Contém 'java'?", trie.Search("java"))
// 	// Big-O: Inserção/Busca O(m) onde m = tamanho da palavra
// }
